...
export function ShelbyProvider({
  children,
}: {
  children: ReactNode;
}) {
    ...
    async ({
      ...
      try {
        ...
        const result = await uploadToShelby({
          ...
          file,
          account: {
            address: accountAddressStr,
          },

          signAndSubmitTransaction: async (tx: any) => {
            console.log("TX FROM SHELBY:", tx);
        ...
        const newFile: ShelbyFile = {
          ...
          downloadUrl: `https://api.testnet.shelby.xyz/shelby/v1/blobs/${accountAddressStr}/${file.name}`,
        };
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      const blobs = await listAccountFiles(
        shelbyClient,
...
export function useShelby() {
  ...
  if (!ctx) {
    throw new Error("useShelby must be used inside ShelbyProvider");
  }
